...
/** Port of the 1.21.11 SideChainPartBlock : blocks that link into rows of three. */
public interface SideChainPartBlock {

	CTBSideChainPart getSideChainPart(BlockState state);

	BlockState setSideChainPart(BlockState state, CTBSideChainPart newPart);
	...
	default List<BlockPos> getAllBlocksConnectedTo(LevelAccessor level, BlockPos pos) {
		...
		results.add(pos);
		this.addBlocksConnectingTowards(neighbors::left, CTBSideChainPart.LEFT, results::addFirst);
		this.addBlocksConnectingTowards(neighbors::right, CTBSideChainPart.RIGHT, results::addLast);
		return results;
	}

	private void addBlocksConnectingTowards(IntFunction<Neighbor> getNeighbor, CTBSideChainPart endPart,
			Consumer<BlockPos> accumulator) {
	...
	default void updateSelfAndNeighborsOnPoweringUp(LevelAccessor level, BlockPos pos, BlockState state, BlockState oldState) {
		...
		Neighbors neighbors = this.getNeighbors(level, pos, this.getFacing(state));
		CTBSideChainPart newPartForSelf = CTBSideChainPart.UNCONNECTED;
		int chainOnTheLeft = neighbors.left().isConnectable()
				? this.getAllBlocksConnectedTo(level, neighbors.left().pos()).size() : 0;
		int chainOnTheRight = neighbors.right().isConnectable()
				? this.getAllBlocksConnectedTo(level, neighbors.right().pos()).size() : 0;
		int currentChainLength = 1;
		if (this.canConnect(chainOnTheLeft, currentChainLength)) {
			newPartForSelf = newPartForSelf.whenConnectedToTheLeft();
			neighbors.left().connectToTheRight();
			currentChainLength += chainOnTheLeft;
		}
		if (this.canConnect(chainOnTheRight, currentChainLength)) {
	...
	private void setPart(LevelAccessor level, BlockPos pos, CTBSideChainPart newPart) {
		BlockState state = level.getBlockState(pos);
	...
	interface Neighbor {
		...
		boolean connectsTowards(CTBSideChainPart endPart);
	...
	record EmptyNeighbor(BlockPos pos) implements Neighbor {
		...
		@Override
		public boolean connectsTowards(CTBSideChainPart endPart) {
			return false;
		}
	}

	record SideChainNeighbor(LevelAccessor level, SideChainPartBlock block, BlockPos pos, CTBSideChainPart part)
			implements Neighbor {
		@Override
		public boolean isConnectable() {
			return true;
	...
	record Neighbors(SideChainPartBlock block, LevelAccessor level, Direction facing, BlockPos center,
		...
		private Neighbor createNewNeighbor(BlockPos pos) {
			...
			CTBSideChainPart part = this.isConnectableToThisBlock(neighbor) ? this.block.getSideChainPart(neighbor) : null;
			return part == null ? new EmptyNeighbor(pos) : new SideChainNeighbor(this.level, this.block, pos, part);
		}

		private Neighbor getOrCreateNeighbor(Direction direction, Integer steps) {
