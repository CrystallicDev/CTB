...
public class SpearGameTests {
	...
	// a couched iron spear at dash speed must hurt the target, speeds come
	// from consecutive position samples like the real per tick calls
	...
	public static void kineticStabHurtsAtSpeed(GameTestHelper helper) {
		...
		SpearItem item = (SpearItem) spear.getItem();
		player.setPos(helper.absoluteVec(new Vec3(0.8, 2.0, 3.5)));
		item.kineticTick(player, spear, 39);
		player.setPos(helper.absoluteVec(new Vec3(1.5, 2.0, 3.5)));
		item.kineticTick(player, spear, 40);
